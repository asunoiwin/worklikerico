...
# ============================================================
if [[ "$TOOL_NAME" =~ ^(Bash|exec_command|functions\.exec_command)$ ]] && { [ "$CURRENT_STATE" = "T1_PASS" ] || [ "$CURRENT_STATE" = "T2_PASS" ]; }; then
...
# ============================================================
if [[ "$TOOL_NAME" =~ ^(Bash|exec_command|functions\.exec_command)$ ]] && [ "$DTL_DISABLE_SLOP_GATE" != "1" ]; then
    CMD=$(echo "$TOOL_INPUT_RAW" | python3 -c "import json,sys;d=json.load(sys.stdin);print(d.get('command') or d.get('cmd') or '')" 2>/dev/null || echo "")
...
# ============================================================
if [[ "$TOOL_NAME" =~ ^(Bash|exec_command|functions\.exec_command)$ ]] && [ "$DTL_DISABLE_DOUBT_GATE" != "1" ]; then
    CMD=$(echo "$TOOL_INPUT_RAW" | python3 -c "import json,sys;d=json.load(sys.stdin);print(d.get('command') or d.get('cmd') or '')" 2>/dev/null || echo "")
...
# ============================================================
if [[ "$TOOL_NAME" =~ ^(Bash|exec_command|functions\.exec_command)$ ]] && [ "$DTL_DISABLE_LINT_GATE" != "1" ]; then
    CMD=$(echo "$TOOL_INPUT_RAW" | python3 -c "import json,sys;d=json.load(sys.stdin);print(d.get('command') or d.get('cmd') or '')" 2>/dev/null || echo "")
    if echo "$CMD" | grep -qE '^\s*git\s+commit\b'; then
