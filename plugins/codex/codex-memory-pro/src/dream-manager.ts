...
import { memoryProPath } from './runtime-paths.js';
...
const DREAM_MD_FILE = memoryProPath('dream.md');
const TRAIL_STATE_FILE = memoryProPath('dream-trail-state.json');
const LAST_RUN_FILE = memoryProPath('dream-last-run.json');
const DREAM_LOCK_FILE = memoryProPath('dream-cycle.lock');
...
function saveTrailState(state: TrailState): void {
  ...
  } catch (err) { console.error(`[codex-memory-pro] dream trail 状态落盘失败: ${err instanceof Error ? err.message : err}`); }
}
...
export function saveLastRunState(state: LastRunState): void {
  ...
  } catch (err) { console.error(`[codex-memory-pro] dream last-run 落盘失败（可能致重复晋升）: ${err instanceof Error ? err.message : err}`); }
}
...
function acquireDreamCycleLock(): number | null {
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    try {
      ...
      try {
        if (Date.now() - statSync(DREAM_LOCK_FILE).mtimeMs <= DREAM_LOCK_STALE_MS) return null;
        unlinkSync(DREAM_LOCK_FILE);
      } catch (statErr: any) {
        if (statErr?.code !== 'ENOENT') return null;
      }
    }
  }
  return null;
...
export async function withDreamCycleLock<T>(work: () => Promise<T>): Promise<T | null> {
  const fd = acquireDreamCycleLock();
  if (fd === null) return null;
  try {
    return await work();
  } finally {
    try { closeSync(fd); } catch {}
    try { unlinkSync(DREAM_LOCK_FILE); } catch {}
  }
}
...
export async function recoverMissedPhases(
  ...
  if (changed) saveLastRunState(state);
  return { recovered };
}
...
export async function runDreamMaintenance(
  ...
  if (opts.cleanup !== false) {
    try {
      ...
      console.error(`[codex-memory-pro] dream 维护-清理失败: ${err instanceof Error ? err.message : err}`);
    }
  }
...
export async function runScheduledDreamCycle(
  store: MemoryStore,
  embedder: Embedder,
  config: DreamConfig = DEFAULT_CONFIG,
  intervalMs = 30 * 60 * 1000
): Promise<{ promotion: Awaited<ReturnType<typeof promoteMemoriesFromStore>>; maintenance: MaintenanceReport | null } | null> {
  return withDreamCycleLock(async () => {
    const state = loadLastRunState();
    if (!needsRecovery(state.light, intervalMs)) return null;
    const promotion = await promoteMemoriesFromStore(store, { ...config, mode: 'light' });
    state.light = new Date().toISOString();
    saveLastRunState(state);
    const maintenance = await maybeRunMaintenance(store, embedder);
    return { promotion, maintenance };
  });
}
...
const MEMORY_INDEX = memoryProPath('dream-memory-index.md');
const PROMOTED_FILE = memoryProPath('dream-promoted.md');
...
export function syncPromotedToMemoryMd(candidates: PromotionCandidate[]): number {
  ...
  mkdirSync(dirname(PROMOTED_FILE), { recursive: true });
...
`;
  ...
  } catch (err) { console.error(`[codex-memory-pro] MEMORY 索引更新失败: ${err instanceof Error ? err.message : err}`); }
