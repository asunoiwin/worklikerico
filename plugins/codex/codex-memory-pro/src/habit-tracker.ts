...
const HABIT_FILE = memoryProPath('habit-candidates.json');
const HABIT_ROLLUP_FILE = memoryProPath('instinct-rollup.md');
...
function saveStats(stats: Map<string, MemoryRecallStats>): void {
  ...
  try {
    ...
    console.error(`[codex-memory-pro] 习惯统计落盘失败: ${err instanceof Error ? err.message : err}`);
  }
}
