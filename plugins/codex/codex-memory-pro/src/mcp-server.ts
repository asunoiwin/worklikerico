...
import { refreshMemoryAtlas, ensureMemoryAtlasFresh, getAtlasHintsForQuery } from "./memory-atlas.js";
...
import { promoteMemoriesFromStore, recoverMissedPhases, getDreamStats, readDreamTrail, DEFAULT_CONFIG as DREAM_DEFAULT_CONFIG, type DreamConfig, runDreamMaintenance, maybeRunMaintenance, runScheduledDreamCycle, withDreamCycleLock } from "./dream-manager.js";
import { runDailyReorganization } from "./memory-daily-reorg.js";
import { getMemoryDbPath } from "./runtime-paths.js";
...
async function classifyFactStance(
  ...
  if (!hasStanceLLMConfig()) {
    console.error("[codex-memory-pro] stance LLM 未配置，跳过 metadata.stance 判定");
    return null;
  }
  ...
  if (!stance) {
    console.error(`[codex-memory-pro] stance LLM 判定失败或输出非法，factKey=${factKey}`);
    return null;
  }
  return stance;
}
...
server.tool(
  ...
  async ({ query, limit, scope, category }) => {
    ...
    if (kg && kg.getStats().totalNodes > 0) {
      ...
      if (kgCandidates.length > 0) {
        ...
        try {
          ...
          recallPath = 'fts_fallback';
          console.error(
            `[codex-memory-pro] KG 向量精排不可用，降级到本地检索: ${error instanceof Error ? error.message : String(error)}`
          );
        }
      }
    }
    ...
    const ftsOnly = results.every(r => r.sources?.bm25 && !r.sources?.vector);
    const pathLabel = recallPath === 'kg_routed' ? 'KG路由' : ftsOnly ? 'FTS降级' : '向量回退';
...
server.tool(
  ...
  async () => {
    ...
    const atlasStatus = await ensureMemoryAtlasFresh(store).catch(() => null);
  ...
  async ({ dryRun, batchSize, scope }) => {
    ...
    if (!dryRun) {
      for (let i = 0; i < plans.length; i += safeBatchSize) {
        ...
        for (const plan of batch) {
          ...
          try {
            ...
            console.error(`[codex-memory-pro] factKey 回填失败 id=${plan.entry.id}:`, error instanceof Error ? error.message : String(error));
            failed += 1;
            failedIds.push(plan.entry.id);
          }
        }
      ...
      if (!hasStanceLLMConfig() && llmCallCount > 0) {
        console.error("[codex-memory-pro] stance LLM 未配置，跳过所有 metadata.stance 回填");
        ...
        for (const [, members] of collisionGroups) {
          for (const member of members) {
            ...
            try {
              ...
              console.error(`[codex-memory-pro] stance 回填失败 id=${member.entry.id}:`, error instanceof Error ? error.message : String(error));
              failed += 1;
              failedIds.push(member.entry.id);
            }
          }
      ...
      if (kg) {
        try {
          ...
          console.error("[codex-memory-pro] fact 回填后 KG 重建失败:", error instanceof Error ? error.message : String(error));
        }
      }
    }
...
server.tool(
  ...
  async ({ action, query }) => {
    ...
    if (action === "query" && query) {
      ...
      const hints = getAtlasHintsForQuery(query, atlas);
...
async function main() {
  if (!EMBEDDING_API_KEY) {
    console.error("[codex-memory-pro] 警告：EMBEDDING_API_KEY 未设置");
  }
  await store.init();
  ...
  try {
    ...
    const kgStats = knowledgeGraph.getStats();
    console.error(`[codex-memory-pro] KG built: ${kgStats.totalNodes} nodes, ${kgStats.totalEdges} edges, ${kgStats.entityKeys} entities`);
  } catch (err) {
    console.error(`[codex-memory-pro] KG build failed: ${err}`);
  }

  try {
    const atlas = await ensureMemoryAtlasFresh(store);
    console.error(`[codex-memory-pro] Atlas ready: ${atlas.totalIndexed || 0} indexed`);
  } catch (err) {
    console.error(`[codex-memory-pro] Atlas refresh failed: ${err}`);
  }

  // Dream 恢复与维护：同一运行目录内仅允许一个 MCP 进程执行
  try {
    await withDreamCycleLock(async () => {
      const dreamRecovery = await recoverMissedPhases(store, DREAM_DEFAULT_CONFIG);
      if (dreamRecovery.recovered.length > 0) {
        console.error(`[codex-memory-pro] Dream recovery: ${dreamRecovery.recovered.join(', ')}`);
      }
      const m = await maybeRunMaintenance(store, embedder);
      if (m) {
        console.error(`[codex-memory-pro] Dream maintenance: cleanup(del=${m.cleanup?.deleted ?? 0},dedup=${m.cleanup?.deduped ?? 0}) compact=${m.compact?.ok ? 'ok' : 'skip/fail'}`);
      }
    });
  } catch (err) {
    console.error(`[codex-memory-pro] Dream recovery/maintenance failed: ${err}`);
  }
  ...
  dreamTimer = setInterval(async () => {
    try {
      const cycle = await runScheduledDreamCycle(store, embedder, DREAM_DEFAULT_CONFIG, DREAM_INTERVAL_MS);
      if (cycle?.promotion.written) {
        console.error(`[codex-memory-pro] Dream auto-promote: ${cycle.promotion.written} written, ${cycle.promotion.skipped} skipped`);
      }
      ...
      console.error(`[codex-memory-pro] Dream auto-promote failed: ${err}`);
    }
  }, DREAM_INTERVAL_MS);
  dreamTimer.unref();
  console.error(`[codex-memory-pro] Dream timer: light promotion every 30min + daily maintenance`);

  const transport = new StdioServerTransport();
  transport.onclose = () => gracefulShutdown('transport-close');
  await server.connect(transport);
  console.error("[codex-memory-pro] MCP Server v2.0.0 started");
  console.error(`[codex-memory-pro] DB: ${DB_PATH}, Model: ${EMBEDDING_MODEL}`);
  console.error(`[codex-memory-pro] Features: atlas, habits, capture${autoCapture.isLLMEnabled ? `(LLM:${autoCapture.captureModel})` : '(keyword-only)'}, cleanup, journal, audit, dream`);
}

main().catch(err => {
  console.error("[codex-memory-pro] Fatal:", err);
  process.exit(1);
});

// 优雅退出兜底：终端关闭/Ctrl+C 时只清理进程内资源。
// Habits/Dream 在各自写入点已经落盘，退出时重复写会制造跨 MCP 进程竞争。
function gracefulShutdown(signal: string) {
  ...
  console.error(`[codex-memory-pro] ${signal} received, closing cleanly...`);
  process.exit(0);
}
...
process.stdin.once('end', () => gracefulShutdown('stdin-end'));
