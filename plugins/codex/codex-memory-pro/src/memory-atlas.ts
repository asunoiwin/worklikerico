...
const ATLAS_FILE = memoryProPath('memory-atlas.json');
...
export async function ensureMemoryAtlasFresh(store: MemoryStore): Promise<Record<string, unknown>> {
  const currentVersion = await store.version();
  const atlas = getMemoryAtlasStatus();
  if (atlas && Number(atlas.storeVersion) === currentVersion) return atlas;
  return refreshMemoryAtlas(store);
}
...
export async function refreshMemoryAtlas(store: MemoryStore): Promise<Record<string, unknown>> {
  const storeVersion = await store.version();
