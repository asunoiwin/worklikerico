...
export class MemoryRetriever {
  ...
  private async hybridRetrieval(
    ...
    try {
      ...
      console.error(
        `[codex-memory-pro] query embedding 不可用，降级到本地 BM25: ${error instanceof Error ? error.message : String(error)}`
      );
      return this.bm25OnlyRetrieval(query, limit, scopeFilter, category);
    }
  ...
  private async bm25OnlyRetrieval(
    ...
    const poolSize = Math.max(this.config.candidatePoolSize, limit * 2);
    const results = await this.store.bm25Search(query, poolSize, scopeFilter);
    const filtered = category ? results.filter(r => r.entry.category === category) : results;
    const mapped: RetrievalResult[] = filtered.map((r, i) => ({
      ...r,
      score: clamp01(Math.max(r.score, 0.5), 0.1),
      sources: {
        bm25: { score: r.score, rank: i + 1 },
        fused: { score: clamp01(Math.max(r.score, 0.5), 0.1) },
      },
    }));
    return this.postProcess(mapped, limit);
  }
