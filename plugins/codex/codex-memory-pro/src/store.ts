...
export class MemoryStore {
  ...
  async init(): Promise<void> {
    ...
    try {
      ...
      if (!this._hasFts) {
        console.error(`[codex-memory-pro] FTS 索引创建失败，回退到纯向量模式: ${msg}`);
      }
    }
  }
  ...
  private async withLiveTable<T>(fn: (table: LanceDB.Table) => Promise<T>): Promise<T> {
    ...
    try {
      ...
      console.error(`[codex-memory-pro] 表句柄陈旧（另一进程已回收旧版本），重开句柄重试`);
      this.table = await this.db.openTable("memories");
      return await fn(this.table);
    }
  }
  ...
  async optimize(retainMs = 2 * 60 * 1000): Promise<{ ok: boolean; error?: string }> {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : String(err);
      console.error(`[codex-memory-pro] optimize 失败: ${msg}`);
  ...
  async incrementRecallBatch(ids: string[]): Promise<void> {
    ...
    try {
      ...
      console.error(`[codex-memory-pro] incrementRecallBatch 失败: ${err instanceof Error ? err.message : err}`);
    }
  }
