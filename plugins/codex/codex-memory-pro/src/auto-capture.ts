...
export async function llmJsonAnalyze<T = Record<string, unknown>>({
  ...
  console.error(`[codex-memory-pro] llmJsonAnalyze 失败(${model}): ${lastReason}`);
  return null;
}
  ...
  async capture(
    ...
    if (category) {
      ...
      try {
        ...
        console.error(`[codex-memory-pro] 自动捕获写库失败(${category}): ${err instanceof Error ? err.message : err}`);
        return null;
      }
    }
      ...
      for (const pattern of patterns) {
        if (lower.includes(pattern.toLowerCase())) {
          ...
          try {
            ...
            console.error(`[codex-memory-pro] 自动捕获写库失败(${type}): ${err instanceof Error ? err.message : err}`);
            return null;
          }
        }
      }
    ...
    if (this.config.llmEnabled && normalizedContent.length >= 20) {
      ...
      if (analysis?.capture && analysis.type && analysis.summary) {
        ...
        try {
          ...
          console.error(`[codex-memory-pro] 自动捕获写库失败(llm:${analysis.type}): ${err instanceof Error ? err.message : err}`);
          return null;
        }
      }
    }
