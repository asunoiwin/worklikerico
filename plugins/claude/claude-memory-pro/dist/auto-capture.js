...
用户输入：`;
export async function llmJsonAnalyze({ apiKey, baseURL, model, systemPrompt, userPrompt, maxTokens = 2048, timeoutMs = 20000, }) {
    if (!apiKey)
        return null;
    ...
    for (let attempt = 1; attempt <= 2; attempt++) {
        ...
        try {
            ...
            if (!content) {
                lastReason = `empty_content(finish=${finish})`;
                continue;
            } // 思考吃光预算，重试
...
async function llmAnalyze(text, apiKey, baseURL, model) {
    return llmJsonAnalyze({
        apiKey,
        baseURL,
        model,
...
// Helpers
// ============================================================================
function isCaptureNoise(content, source) {
    const normalized = content.trim();
    if (!normalized)
        return true;
    if (source === 'before_agent_start' || source === 'session_start')
        return true;
    if (/^Current time:/i.test(normalized))
        return true;
    if (/^Relevant memory:/i.test(normalized))
        return true;
    if (/\[Internal task completion event\]/i.test(normalized))
        return true;
    // 太短的内容不值得分析
...
function buildCaptureText(content, context) {
...
export class AutoCaptureEngine {
    ...
    embedder;
    config;
    context = { sessionId: 'unknown', scope: 'global' };
    // LLM 配置（从环境变量读取）
    llmApiKey;
    llmBaseURL;
    llmModel;
    ...
    async capture(content, category, importance, overrides) {
        ...
        if (isCaptureNoise(normalizedContent, overrides?.source || this.context.source))
            return null;
        ...
        const buildMetadata = (captureKind, llmUsed, textForKey, memoryCategory) => JSON.stringify(metadataWithFactKey({
            text: textForKey,
        ...
        // 路径 1：强制指定 category
        if (category) {
            const imp = importance || this.config.importance[category] || 0.5;
        ...
        for (const [type, patterns] of Object.entries(this.config.patterns)) {
            for (const pattern of patterns) {
                if (lower.includes(pattern.toLowerCase())) {
                    const imp = this.config.importance[type] || 0.5;
                    const memoryText = buildCaptureText(normalizedContent, effectiveContext);
    ...
    analyze(content) {
        ...
        for (const [type, patterns] of Object.entries(this.config.patterns)) {
            for (const pattern of patterns) {
                if (lower.includes(pattern.toLowerCase())) {
                    return { type, importance: this.config.importance[type] || 0.5 };
                }
            }
        }
        return null;
