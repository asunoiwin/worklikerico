...
function groupByEntityKey(entries) {
...
function negatorCount(s) {
    return (s.match(/[不别勿未无]/g) || []).length;
}
export function detectSimpleContradiction(newer, older) {
...
// ============================================================================
async function processEntityGroups(groups, store) {
...
// Expiry Detection
// ============================================================================
function findExpiredMemories(entries) {
    const now = Date.now();
    const maxAgeMs = MAX_AGE_DAYS * 24 * 60 * 60 * 1000;
    return entries
        .filter(e => {
        // 有召回记录的不过期（热记忆保护）
        if ((e.recallCount ?? 0) > 0)
            return false;
        try {
            if (e.metadata) {
...
// Merge Log
// ============================================================================
function appendMergeLog(result, conflicts) {
    try {
        let log = [];
        if (existsSync(MERGE_LOG_FILE)) {
            try {
                log = JSON.parse(readFileSync(MERGE_LOG_FILE, 'utf8'));
            }
            catch { }
        }
        log.push({
...
// ============================================================================
export async function runDailyReorganization(store, atlasBuildFn, kgBuildFn) {
    ...
    for (const id of expiredIds) {
        try {
            await store.updateEntryExpired(id);
        }
        catch { }
    }
    ...
    console.info(`[daily-reorg] 完成 ${result.duration}ms: ` +
        `atlas=${atlasEntries}, KG=${kgRebuilt}, ` +
        `${groups.length} 实体组, ` +
        `${contradictions} 矛盾, ${superseded} 取代, ${expiredIds.length} 过期`);
    return result;
}
//# sourceMappingURL=memory-daily-reorg.js.map