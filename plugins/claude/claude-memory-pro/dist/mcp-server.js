...
import { getVectorIssue, MemoryStore } from "./store.js";
...
import { KnowledgeGraphManager, setKG, getKG, isFactKeyCategory, metadataWithFactKey, normalizeFactKey, parseMemoryMetadata, } from "./knowledge-graph.js";
import { promoteMemoriesFromStore, recoverMissedPhases, getDreamStats, readDreamTrail, DEFAULT_CONFIG as DREAM_DEFAULT_CONFIG, saveLastRunState, loadLastRunState, runDreamMaintenance, maybeRunMaintenance } from "./dream-manager.js";
...
const CATEGORIES = ["preference", "fact", "decision", "entity", "other", "task", "lesson"];
// ============================================================================
// Helpers
// ============================================================================
function cosineSimVectors(a, b) {
...
function mmrDedup(scored, limit, threshold = 0.85) {
    const selected = [];
    for (const candidate of scored) {
        const tooSimilar = selected.some(s => {
            const sVec = Array.from(s.entry.vector);
            const cVec = Array.from(candidate.entry.vector);
...
function clamp01(v, fallback = 0.7) {
...
function formatResults(results) {
    ...
    return results.map((r, i) => {
        ...
        const metaParts = [];
        if (meta.taskId)
            metaParts.push(`task=${meta.taskId}`);
        if (meta.source)
...
async function scanVectorHealth(scope, pageSize = 500) {
...
function normalizeLLMStance(value) {
    if (typeof value !== "string")
        return null;
    const normalized = value.trim().toLowerCase();
    if (normalized === "affirm" || normalized === "positive")
        return "affirm";
    if (normalized === "negate" || normalized === "negative")
        return "negate";
    if (normalized === "neutral")
        return "neutral";
    return null;
}
function hasStanceLLMConfig() {
    return Boolean(CAPTURE_API_KEY && CAPTURE_BASE_URL && CAPTURE_MODEL);
}
async function scanFactEntries(scope, pageSize = 500) {
    const scopeFilter = scope ? [scope] : undefined;
    const totalRows = await store.count(scopeFilter);
    const safePageSize = clampInt(pageSize, 1, 1000);
    const entries = [];
    for (let offset = 0; offset < totalRows; offset += safePageSize) {
        ...
        for (const entry of batch) {
            if (isFactKeyCategory(entry.category))
                entries.push(entry);
        }
    }
    return entries;
}
async function findEntriesByFactKey(factKey, options = {}) {
    const target = normalizeFactKey(factKey);
    if (!target)
        return [];
    const entries = await scanFactEntries(options.scope, 500);
    return entries.filter(entry => {
        if (options.excludeId && entry.id === options.excludeId)
            return false;
        const meta = parseMemoryMetadata(entry);
        return normalizeFactKey(meta.factKey) === target;
    });
}
async function classifyFactStance(text, factKey, peers) {
...
async function classifyContradictionPair(a, b, model = CAPTURE_MODEL) {
    const result = await llmJsonAnalyze({
        apiKey: CAPTURE_API_KEY,
        baseURL: CAPTURE_BASE_URL,
        model,
...
async function buildWriteMetadataWithFactStance(text, category, scope, metadata, options = {}) {
...
// -- memory_recall --
server.tool("memory_recall", "语义检索长期记忆。KG 多维路由（实体/时间/因果/分类）→ 向量精排。KG 为空时回退到纯向量检索。", {
    ...
    if (shouldSkipRetrieval(query)) {
        return { content: [{ type: "text", text: "查询过短或为系统命令，已跳过检索。" }] };
    }
    const scopeFilter = scope ? [scope] : undefined;
    let results = [];
    ...
    const kg = getKG();
    if (kg)
        await kg.ensureFresh(); // store 变更（含其它会话进程的写）则重建，避免陈旧/缺新
    if (kg && kg.getStats().totalNodes > 0) {
        ...
        if (kgCandidates.length > 0) {
            ...
            for (const kgResult of kgCandidates) {
                ...
                const combinedScore = Math.min(kgResult.score * 0.4 + vectorSim * 0.4 + (entry.importance ?? 0.5) * 0.2, 1.0);
                scored.push({
    ...
    return { content: [{ type: "text", text: `[${pathLabel}] 找到 ${results.length} 条记忆：\n\n${formatResults(results)}` }] };
});
// -- memory_store --
server.tool("memory_store", "保存重要信息到长期记忆。自动去重、噪音过滤，支持分类和重要性评分。", {
    ...
    if (isNoise(text)) {
        return { content: [{ type: "text", text: "跳过：文本被识别为噪音" }] };
    }
    ...
    const metadata = await buildWriteMetadataWithFactStance(text.slice(0, 5000), category, scope, { source: "manual_store", storedAt: new Date().toISOString() });
...
// -- memory_forget --
server.tool("memory_forget", "删除指定记忆。支持按 ID 直接删除或按语义搜索后删除。", {
    ...
    if (memoryId) {
        ...
        if (deleted)
            removeFromHabits(memoryId);
        return { content: [{ type: "text", text: deleted ? `已删除记忆 ${memoryId}` : `未找到记忆 ${memoryId}` }] };
    }
    if (query) {
        ...
        return { content: [{ type: "text", text: `找到 ${results.length} 条候选，请指定 memoryId：\n${list}` }] };
    }
    return { content: [{ type: "text", text: "请提供 query 或 memoryId。" }] };
});
// -- memory_update --
server.tool("memory_update", "更新已有记忆的内容、重要性或分类。保留原始时间戳。", {
    ...
    if (!text && importance === undefined && !category) {
        return { content: [{ type: "text", text: "至少提供一项更新。" }] };
    }
    const updates = {};
    if (text) {
        if (isNoise(text))
            return { content: [{ type: "text", text: "跳过：噪音文本。" }] };
        updates.text = text;
        updates.vector = await embedder.embedPassage(text);
    }
    if (importance !== undefined)
        updates.importance = clamp01(importance);
    if (category)
    ...
    if (text || category) {
        ...
        if (existing) {
            ...
            const nextMeta = await buildWriteMetadataWithFactStance(nextText, nextCategory, existing.scope, parseMemoryMetadata(existing), { excludeId: existing.id });
    ...
    return { content: [{ type: "text", text: `已更新 ${updated.id.slice(0, 8)}：「${updated.text.slice(0, 80)}」` }] };
});
// -- memory_list --
server.tool("memory_list", "列出最近的记忆，支持按域和分类过滤。", {
    ...
    return { content: [{ type: "text", text: `记忆列表（${entries.length}条）：\n\n${text}` }] };
});
// -- memory_stats --
server.tool("memory_stats", "查看记忆系统完整统计：总数、分布、习惯追踪、知识图谱、捕获队列。", {}, async () => {
...
// -- memory_reindex --
server.tool("memory_reindex", "诊断并修复存量记忆的向量健康。dryRun 默认只统计真实 LanceDB 覆盖率，不写入。", {
...
server.tool("memory_backfill_facts", "为存量 decision/lesson/fact 记忆回填 metadata.factKey；同 factKey 分组成员>=2 时用轻量 LLM 回填 metadata.stance。dryRun 默认只报告。", {
    ...
    for (const plan of plans) {
        if (!groups.has(plan.factKey))
            groups.set(plan.factKey, []);
...
server.tool("memory_scan_contradictions", "用向量近邻召回疑似讲同一事实的记忆对，再用 LLM 判定是否真矛盾。绕开 factKey 精确碰撞死结。dryRun 默认只报告候选规模，不写库。", {
    ...
    if (dryRun) {
        ...
        const preview = candidates.slice(0, 10).map((c, i) => `  ${i + 1}. [${c.score.toFixed(3)}] ${c.a.scope} | ${c.a.text.slice(0, 40)} ↔ ${c.b.text.slice(0, 40)}`);
        if (preview.length > 0)
            lines.push("", "候选对预览（相似度Top10）：", ...preview);
        return { content: [{ type: "text", text: lines.join("\n") }] };
    }
    if (!hasStanceLLMConfig()) {
    ...
    lines.push(`• 实际 LLM 判定：${judged}（失败 ${failed}）`, `• 确认矛盾对（sameFact 且 contradicts 且置信≥${minConfidence}）：${contradictions.length}`, persist
        ? `• 已标废较旧记忆：${superseded} 条（newer-wins，已重建 KG，召回即过滤）`
        : "（本工具仅验证收益，不写库；加 persist=true 可标废较旧的一条）");
    if (contradictions.length > 0) {
        const detail = contradictions.slice(0, 20).map((c, i) => `  ${i + 1}. [置信${c.confidence.toFixed(2)} 相似${c.score.toFixed(2)}]\n     A(${c.a.scope}): ${c.a.text.slice(0, 90)}\n     B(${c.b.scope}): ${c.b.text.slice(0, 90)}`);
...
// ============================================================================
// -- memory_atlas (知识图谱) --
server.tool("memory_atlas", "构建或查看记忆知识图谱。分析记忆生成聚类、锚点和关联边。", {
    ...
    if (action === "refresh") {
        ...
        const lines = [
            ...
            ``, `主要聚类：`,
            ...clusters.slice(0, 10).map((c) => `  • ${c.label} (${c.count}条) — ${c.topTokens?.slice(0, 5).join(', ')}`),
            ``, `关联：`,
            ...edges.slice(0, 10).map((e) => `  • ${e.from} <-> ${e.to} (${e.weight}, ${e.reason})`),
        ];
        return { content: [{ type: "text", text: lines.join("\n") }] };
    }
    if (action === "query" && query) {
        ...
        if (hints.clusterKeys.length === 0) {
            return { content: [{ type: "text", text: "未找到相关聚类。先 refresh 生成图谱。" }] };
        }
    ...
    if (!atlas)
        return { content: [{ type: "text", text: "知识图谱未生成。用 action='refresh' 构建。" }] };
    const clusters = Array.isArray(atlas.clusters) ? atlas.clusters : [];
    return { content: [{ type: "text", text: `知识图谱：${atlas.generatedAt}\n• ${atlas.totalIndexed}条，${clusters.length}个聚类\n${clusters.slice(0, 8).map((c) => `  • ${c.label} (${c.count})`).join('\n')}` }] };
});
// -- memory_habits (记忆晋升) --
server.tool("memory_habits", "记忆晋升系统。追踪高频召回，按 retain->reinforce->promote 晋升，写入 instinct-rollup.md。", {
    ...
    if (action === "refresh") {
        ...
        return { content: [{ type: "text", text: `已刷新 instinct-rollup.md\n• 总: ${s.total}, promote: ${s.promote}, reinforce: ${s.reinforce}` }] };
    }
    if (action === "candidates") {
        const c = generateHabitCandidates();
        if (c.length === 0)
            return { content: [{ type: "text", text: "暂无候选。" }] };
        ...
        return { content: [{ type: "text", text: `候选（${c.length}）：\n${lines.join("\n")}` }] };
    }
    if (action === "instincts") {
        const ctx = buildInstinctContext();
        return { content: [{ type: "text", text: ctx || "暂无工作先验。" }] };
    }
    const s = getHabitSummary();
    return { content: [{ type: "text", text: `习惯追踪：总${s.total}, promote=${s.promote}, reinforce=${s.reinforce}, retain=${s.retain}` }] };
});
// -- memory_capture (自动捕获) --
server.tool("memory_capture", "自动分析并捕获重要内容。关键词匹配分类（task/rule/decision/correction/preference）。", {
    ...
    if (!result)
        return { content: [{ type: "text", text: "未触发捕获。" }] };
    if (result.kind === 'redirect') {
        return { content: [{ type: "text", text: `⚠️ 已拒绝（${result.redirect}）：${result.hint}` }] };
    }
    const method = result.llmUsed ? '（LLM 分析）' : '（关键词匹配）';
    return { content: [{ type: "text", text: `已捕获${method}：${result.type}，重要性=${result.importance}` }] };
});
// -- memory_cleanup --
server.tool("memory_cleanup", "清理记忆：删噪音、去重、摘要压缩。", {
    ...
    return { content: [{ type: "text", text: `清理完成：扫描${r.scanned}，删噪${r.deleted}，去重${r.deduped}，压缩${r.cleaned}` }] };
});
// -- memory_audit --
server.tool("memory_audit", "记忆系统健康审计。", {}, async () => {
    const r = await auditEngine.runAudit();
    return { content: [{ type: "text", text: `审计：总${r.stats.total_memories}，问题${r.stats.issues}` }] };
});
// -- memory_journal (捕获队列) --
server.tool("memory_journal", "捕获队列管理：查看待处理条目和统计。", {
    action: z.enum(["stats", "pending", "prune"]).default("stats").describe("stats/pending/prune"),
}, async ({ action }) => {
    if (action === "pending") {
        const p = captureJournal.listPending(20);
        if (p.length === 0)
            return { content: [{ type: "text", text: "无待处理。" }] };
        return { content: [{ type: "text", text: `待处理(${p.length})：\n${p.map((e, i) => `${i + 1}. ${e.content.slice(0, 80)}...`).join("\n")}` }] };
    }
    if (action === "prune") {
        captureJournal.prune();
        return { content: [{ type: "text", text: "已清理。" }] };
    }
    const s = captureJournal.stats();
    return { content: [{ type: "text", text: `队列：总${s.total}，待处理${s.pending}，已捕获${s.captured}` }] };
});
// -- memory_kg (Knowledge Graph) --
server.tool("memory_kg", "知识图谱（KG）管理。KG 是记忆的结构化索引，包含实体节点、5种关系边（causal/temporal/subject/category/contradicts）和矛盾检测。", {
    ...
    if (action === "query" && query) {
        ...
        return { content: [{ type: "text", text: `KG 查询「${query}」（${results.length}条）：\n${lines.join("\n")}` }] };
    }
    if (action === "contradictions") {
        const ids = memoryIds || [];
        if (ids.length === 0)
            return { content: [{ type: "text", text: "请提供 memoryIds 列表。" }] };
        const contradictions = kg.getContradictions(ids);
        if (contradictions.length === 0)
            return { content: [{ type: "text", text: "未检测到矛盾。" }] };
        const lines = contradictions.map(c => `• ${c.a.slice(0, 8)} <-> ${c.b.slice(0, 8)} (权重: ${c.weight})`);
        return { content: [{ type: "text", text: `检测到 ${contradictions.length} 对矛盾：\n${lines.join("\n")}` }] };
    }
    if (action === "debug") {
        const path = kg.writeDebugSnapshot();
        return { content: [{ type: "text", text: `KG 调试快照已写入：${path}` }] };
    }
    // stats
    await kg.ensureFresh().catch(() => { });
    const s = kg.getStats();
    return { content: [{ type: "text", text: `KG 状态：\n• 节点：${s.totalNodes}（${s.supersededNodes} 被取代）\n• 边：${s.totalEdges}\n• 实体：${s.entityKeys}\n• 分类：${s.categories}\n• 构建时间：${s.builtAt || '未构建'}\n• 边分布：subject=${s.edgesByRelation.subject}, temporal=${s.edgesByRelation.temporal}, causal=${s.edgesByRelation.causal}, category=${s.edgesByRelation.category}, contradicts=${s.edgesByRelation.contradicts}` }] };
});
// -- memory_dream (Dream 记忆晋升) --
server.tool("memory_dream", "Dream 记忆晋升系统。三阶段晋升（light/deep/REM），基于召回频率自动提升记忆，写入 dream.md。支持手动触发、查看 trail、日常整理。", {
    ...
    if (action === "run") {
        ...
        const config = { ...DREAM_DEFAULT_CONFIG, mode };
    ...
    if (action === "trail") {
        ...
        if (!trail.trim())
            return { content: [{ type: "text", text: "dream.md 为空，尚无晋升记录。" }] };
        // 只返回最后 2000 字符
    ...
    if (action === "recover") {
        ...
        if (result.recovered.length === 0) {
            return { content: [{ type: "text", text: "无需恢复，所有阶段均在有效期内。" }] };
        }
        return { content: [{ type: "text", text: `已恢复错过的阶段：${result.recovered.join(', ')}` }] };
    }
    if (action === "maintain") {
...
// ============================================================================
// -- task_create --
server.tool("task_create", "创建/更新跨会话持久化任务。同 project 同 subject 未完成任务自动覆盖（不新建）。", {
    ...
    if (existing) {
        ...
        const updates = { text, metadata: JSON.stringify(meta) };
        if (existing.text !== text) {
            updates.vector = await embedder.embedPassage(text.slice(0, 500));
        }
        await store.update(existing.id, updates);
        return { content: [{ type: "text", text: `已更新任务 [${existing.id.slice(0, 8)}]：${subject} → ${status}` }] };
    }
    const vector = await embedder.embedPassage(text.slice(0, 500));
    ...
    return { content: [{ type: "text", text: `已创建任务 [${newEntry.id.slice(0, 8)}]：${subject}（${status}，project=${project}）` }] };
});
// -- task_list --
server.tool("task_list", "列出指定项目的任务。按状态过滤，默认只返未完成。", {
    ...
    if (filtered.length === 0)
        return { content: [{ type: "text", text: `项目 ${project} 无 ${status} 任务` }] };
    const lines = filtered.map((e, i) => {
        const m = JSON.parse(e.metadata || "{}");
        return `${i + 1}. [${m.status}] ${m.subject}${m.description ? " — " + m.description : ""} (${e.id.slice(0, 8)})`;
    });
    return { content: [{ type: "text", text: `项目 ${project} 任务（${filtered.length}）：\n${lines.join("\n")}` }] };
});
// -- lesson_capture --
server.tool("lesson_capture", "记录踩过的坑/反模式/避坑教训。同类教训自动合并 evidence（向量阈值 0.85，比 memory 宽松）。", {
    ...
    if (matched) {
        ...
        return { content: [{ type: "text", text: `已合并到已有教训 [${similar[0].entry.id.slice(0, 8)}]（相似度 ${(similar[0].score * 100).toFixed(0)}%）。证据次数：${newMeta.evidenceCount}` }] };
    }
    const metaBase = {
    ...
    const meta = await buildWriteMetadataWithFactStance(text.slice(0, 5000), "lesson", scope, metaBase);
    const newEntry = await store.store({
    ...
    return { content: [{ type: "text", text: `已记录新教训 [${newEntry.id.slice(0, 8)}]：${pitfall.slice(0, 60)}...` }] };
});
// -- lesson_recall --
server.tool("lesson_recall", "按关键词召回项目教训。优先返回 triggerKeywords 命中的，其次向量相似。", {
    ...
    const matched = candidates.filter(e => {
        try {
            ...
            return keywords.some(k => triggers.some((t) => t.includes(k.toLowerCase()) || k.toLowerCase().includes(t)));
        }
        catch {
            return false;
        }
    ...
    if (matched.length === 0) {
        ...
        if (filtered.length === 0)
            return { content: [{ type: "text", text: `无相关教训（关键词：${keywords.join(",")}）` }] };
        // 方案 C：lesson 召回纳入 habit 频率统计（task 不纳入）
        try {
            recordRecallBatch(filtered.map(r => ({ id: r.entry.id, text: r.entry.text, category: r.entry.category })));
        }
        catch { }
        ...
        return { content: [{ type: "text", text: `教训命中（向量回退，${filtered.length}）：\n${lines.join("\n")}` }] };
    }
    try {
        recordRecallBatch(matched.map(e => ({ id: e.id, text: e.text, category: e.category })));
    }
    catch { }
    ...
    return { content: [{ type: "text", text: `教训命中（关键词精确，${matched.length}）：\n${lines.join("\n")}` }] };
});
// ============================================================================
// Start
// ============================================================================
...
function gracefulShutdown(signal) {
    ...
    try {
        ...
        state._lastSessionEnd = new Date().toISOString();
