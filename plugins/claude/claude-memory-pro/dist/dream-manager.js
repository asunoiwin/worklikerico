...
function memoryEntryToDreamEntry(entry) {
    if ((entry.recallCount ?? 0) <= 0)
        return null;
    // 方案 C：task/lesson 不进 dream 晋升
    // task 是临时工作流（completed 后语义无召回价值）
    // lesson 已有 evidenceCount 自合并加权机制，叠 dream 是双重计分
    if (entry.category === 'task' || entry.category === 'lesson')
        return null;
    return {
        memoryId: entry.id,
...
function scoreCandidate(entry, _thresholds, recencyHalfLifeDays) {
    const now = Date.now();
    ...
    const promotionScore = (Math.min(entry.recallCount / 10, 1) * 0.25) +
        (avgRelevance * 0.30) +
        (queryDiversity * 0.20) +
        (recencyScore * 0.25);
    let promotionTier = 'none';
...
// ============================================================================
export function getPromotionCandidates(entries, config = DEFAULT_CONFIG) {
    const results = [];
    for (const raw of entries) {
        ...
        if (daysSince > config.aging.maxAgeDays)
            continue;
...
function loadTrailState() {
    try {
        if (existsSync(TRAIL_STATE_FILE)) {
            return JSON.parse(readFileSync(TRAIL_STATE_FILE, 'utf8'));
        }
    }
    catch { }
    return { promoted: {} };
...
function saveTrailState(state) {
    try {
        ...
        console.error(`[claude-memory-pro] dream trail 状态落盘失败: ${err instanceof Error ? err.message : err}`);
    }
}
// ============================================================================
// Dream Trail 摘要
// ============================================================================
function summarizeDreamText(text) {
...
// ============================================================================
export function applyPromotions(candidates, config = DEFAULT_CONFIG) {
    let written = 0;
    let skipped = 0;
    const decisions = [];
    if (candidates.length === 0)
        return { written, skipped, phase: config.mode, decisions };
    const trail = loadTrailState();
    const now = new Date().toISOString();
    const phase = config.mode;
    ...
    const tierOrder = { light: 0, deep: 1, rem: 2 };
...
// ============================================================================
export async function promoteMemoriesFromStore(store, config = DEFAULT_CONFIG) {
...
export function saveLastRunState(state) {
    try {
        mkdirSync(dirname(LAST_RUN_FILE), { recursive: true });
        writeFileSync(LAST_RUN_FILE, JSON.stringify(state, null, 2), 'utf8');
    }
    catch (err) {
        console.error(`[claude-memory-pro] dream last-run 落盘失败（可能致重复晋升）: ${err instanceof Error ? err.message : err}`);
    }
}
export function needsRecovery(lastRun, intervalMs) {
    if (!lastRun)
        return true;
    const elapsed = Date.now() - Date.parse(lastRun);
    return elapsed >= intervalMs;
}
export async function recoverMissedPhases(store, config = DEFAULT_CONFIG) {
 ...
 */
export async function runDreamMaintenance(store, embedder, opts = {}) {
 ...
 * 返回 null 表示未到期、本次跳过。
 */
export async function maybeRunMaintenance(store, embedder, intervalMs = MAINTENANCE_INTERVAL_MS) {
    const state = loadLastRunState();
    if (!needsRecovery(state.maintenance ?? null, intervalMs))
        return null;
 ...
 */
export function syncPromotedToMemoryMd(candidates) {
    ...
    const existingIds = new Set((existingContent.match(/<!-- id:([a-f0-9-]+) -->/g) || [])
        .map(m => m.replace('<!-- id:', '').replace(' -->', '')));
    const newEntries = [];
    for (const c of promoted) {
        if (existingIds.has(c.memoryId))
            continue;
...
`;
    ...
    try {
        ...
        console.error(`[claude-memory-pro] MEMORY 索引更新失败: ${err instanceof Error ? err.message : err}`);
    }
    return newEntries.length;
}
export function readDreamTrail() {
    try {
...
//# sourceMappingURL=dream-manager.js.map