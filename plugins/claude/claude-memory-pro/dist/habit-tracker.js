...
function loadStats() {
    const data = readJson(HABIT_FILE, {});
    if (!Array.isArray(data.stats))
        return new Map();
    return new Map(data.stats.map(item => [item.memoryId, item]));
}
function computeVelocity(stats) {
    ...
    if (stats.lastRecallAt < cutoffStr)
        return 0;
...
function determinePromotionTier(stats, velocity) {
    if (velocity >= 5 || stats.recallCount >= 10)
        return 'promote';
    if (velocity >= HABIT_THRESHOLD)
        return 'reinforce';
    return 'retain';
}
function buildReason(stats, tier) {
...
function buildCandidatesFromStats(stats) {
    const candidates = [];
...
function buildRollupMarkdown(stats) {
...
export function recordRecall(memoryId, memoryText, category, timestamp) {
    ...
    if (existing) {
        ...
        if (stats.size >= MAX_TRACKED) {
            ...
            for (const [id, stat] of stats) {
                if (stat.lastRecallAt < oldestTime) {
                    oldestTime = stat.lastRecallAt;
                    oldestId = id;
                }
            }
            if (oldestId)
                stats.delete(oldestId);
        }
        stats.set(memoryId, {
...
export function recordRecallBatch(results) {
...
export function generateHabitCandidates() {
    return buildCandidatesFromStats(loadStats());
}
export function buildInstinctContext(maxCandidates = 5) {
...
export function refreshHabitArtifacts() {
    saveStats(loadStats());
}
...
export function removeFromHabits(memoryId) {
    const stats = loadStats();
