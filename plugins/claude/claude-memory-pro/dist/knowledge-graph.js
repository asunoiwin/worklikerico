...
function textOverlap(a, b, minJaccard = 0.10) {
    return tokenJaccard(a, b) >= minJaccard;
}
export function parseMemoryMetadata(entry) {
    try {
...
export function isFactKeyCategory(category) {
...
function isDiscriminativeHint(token) {
    if (!token || GENERIC_HINT_TOKENS.has(token))
        return false;
    if (/^\d+$/.test(token))
        return false;
    if (/^[a-z0-9_ -]{1,2}$/.test(token))
        return false;
    return true;
}
function addHintValue(tokens, value) {
    const normalized = normalizeHint(value);
    if (!normalized || !isDiscriminativeHint(normalized))
        return;
    tokens.add(normalized);
    for (const token of tokenize(normalized)) {
        if (isDiscriminativeHint(token))
            tokens.add(token);
    }
}
function addHintList(tokens, value) {
    if (!Array.isArray(value))
        return;
    for (const item of value)
        addHintValue(tokens, item);
}
function addLimitedHintList(tokens, value, limit) {
    if (!Array.isArray(value))
        return;
    for (const item of value) {
...
function stableFactKey(prefix, parts) {
    const normalizedParts = parts
        .map(part => normalizeFactKey(part))
        .filter((part) => typeof part === 'string' && isDiscriminativeHint(part));
    if (normalizedParts.length === 0)
        return null;
    return normalizeFactKey(`${prefix}:${normalizedParts.join('|')}`);
}
function textKeyTerms(text, limit = 5) {
    const cleaned = text
        .replace(/\[[^\]]+\]/g, ' ')
...
export function generateFactKeyForMemory(entry) {
...
export function metadataWithFactKey(entry) {
    const meta = parseMemoryMetadata(entry);
    if (!isFactKeyCategory(entry.category) || normalizeFactKey(meta.factKey))
        return meta;
...
function addCausalTerms(tokens, text) {
    const lower = text.toLowerCase();
    for (const term of CAUSAL_TERMS) {
        if (lower.includes(term.toLowerCase()))
            tokens.add(`causal:${term.toLowerCase()}`);
    }
}
function extractHintTokens(entry) {
    const meta = parseMemoryMetadata(entry);
    ...
    if (typeof meta.summary === 'string')
        addCausalTerms(tokens, meta.summary);
...
function normalizeMetadataStance(value) {
    if (typeof value !== 'string')
        return null;
...
function inferStructuredStance(entry) {
    const meta = parseMemoryMetadata(entry);
    const metadataStance = normalizeMetadataStance(meta.stance);
    if (metadataStance)
        return metadataStance;
    const fragments = [
        entry.text,
    ...
    if (!fragments)
        return 'neutral';
    if (hasNegationSignal(fragments))
        return 'negate';
    if (/(使用|采用|保留|作为|启用|合并|注入|确认|决定|采纳|replace with|use |keep |enable |adopt |merge )/i.test(fragments))
        return 'affirm';
    return 'neutral';
}
function contradictionStrength(a, b) {
    ...
    if (stanceA === 'neutral' || stanceB === 'neutral')
        return 0;
    if (stanceA === stanceB)
        return 0;
    if (sameFactKey)
        return 0.9;
    if (sameTaskKey && sameRuleHint)
        return 0.65;
    return 0;
}
function sharedHintStats(a, b) {
    const aHints = new Set(extractHintTokens(a));
    const bHints = new Set(extractHintTokens(b));
    if (aHints.size === 0 || bHints.size === 0)
        return { shared: 0, jaccard: 0, overlapCoefficient: 0 };
    let shared = 0;
    for (const token of aHints) {
        if (bHints.has(token))
            shared++;
    }
...
function causalEdgeWeight(a, b, summaryA, summaryB) {
    const hintStats = sharedHintStats(a, b);
    if (hintStats.shared < MIN_SHARED_CAUSAL_HINTS)
        return 0;
...
function extractEntityKey(entry) {
    const meta = parseMemoryMetadata(entry);
...
function extractSummary(entry, maxLen = 120) {
    try {
        if (entry.metadata) {
            const meta = JSON.parse(entry.metadata);
            if (meta.summary && typeof meta.summary === 'string')
                return meta.summary.slice(0, maxLen);
            const fragments = [
            ...
            if (fragments.length > 0)
                return fragments.join(' | ').slice(0, maxLen);
        }
    }
    catch { /* ignore */ }
...
export class KnowledgeGraphManager {
    ...
    async build() {
        ...
        const entityKeyChain = new Map();
        for (const entry of entries) {
            const ek = extractEntityKey(entry);
            if (!ek)
                continue;
            if (!entityKeyChain.has(ek))
                entityKeyChain.set(ek, []);
        ...
        for (const entry of entries) {
            ...
            if (ek) {
                if (!this.kg.byEntityKey.has(ek))
                    this.kg.byEntityKey.set(ek, []);
                this.kg.byEntityKey.get(ek).push(entry.id);
            }
            if (!this.kg.byCategory.has(entry.category))
                this.kg.byCategory.set(entry.category, []);
    ...
    query(rawQuery, options = {}) {
        ...
        const addCandidate = (id, score, dim, reason) => {
            const node = this.kg.nodes.get(id);
            if (!node)
                return;
            if (!includeSuperseded && node.superseded)
                return;
        ...
        for (const edge of this.kg.edges) {
            if (edge.relation === 'category' || edge.relation === 'contradicts')
                continue;
            const dim = edge.relation === 'temporal' ? 'temporal'
    ...
    writeDebugSnapshot(filePath = join(homedir(), '.claude', 'memory-pro', 'kg-debug-latest.json')) {
...
export function setKG(kg) { _kgInstance = kg; }
export function getKG() { return _kgInstance; }
