...
function parseMetadata(entry) {
    try {
        return JSON.parse(String(entry.metadata || '{}'));
    }
    catch {
        return {};
    }
}
function pickClusterKey(entry, source, tokens) {
    ...
    if (tokens.length > 0)
        return `topic:${tokens.slice(0, 2).join('+')}`;
...
function overlapRatio(a, b) {
    ...
    for (const token of b) {
        if (setA.has(token) || a.some(left => left.includes(token) || token.includes(left)))
            hits++;
    }
    return hits / Math.max(a.length, b.length);
}
function buildClusterLabel(key, tokens) {
    if (key.startsWith('task:'))
        return key.replace(/^task:/, 'task ');
    if (key.startsWith('scope:'))
        return key.replace(/^scope:/, 'scope ');
...
export function getMemoryAtlasStatus() {
    if (!existsSync(ATLAS_FILE))
        return null;
    try {
        return JSON.parse(readFileSync(ATLAS_FILE, 'utf8'));
    }
    catch {
        return null;
    }
}
export function getAtlasHintsForQuery(query, atlas = getMemoryAtlasStatus()) {
    if (!atlas || !query)
        return { clusterKeys: [], anchorIds: [], summary: [] };
    ...
    const ranked = clusters
        ...
        .slice(0, 2);
    return {
        clusterKeys: ranked.map((e) => String(e.cluster.key)),
        anchorIds: [...new Set(ranked.flatMap((e) => Array.isArray(e.cluster.anchorIds) ? e.cluster.anchorIds.slice(0, 6) : []))],
        summary: ranked.map((e) => `${e.cluster.label} (${e.cluster.count})`),
    };
}
export async function refreshMemoryAtlas(store) {
    ...
    for (const entry of entries) {
        if (isNoise(entry.text) || isOperationalRecap(entry.text))
            continue;
        const metadata = parseMetadata(entry);
...
//# sourceMappingURL=memory-atlas.js.map